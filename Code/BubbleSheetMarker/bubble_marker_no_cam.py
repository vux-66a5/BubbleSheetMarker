...
def load_image(image_path):
    ...
    if image is None:
        print(f"Không thể mở ảnh từ {image_path}")
        exit()
    return image
...
def main():
    ...
    # Thay vì mở video stream, giờ ta sẽ mở ảnh tĩnh
    image_path = 'IMG_20241221_165621.jpg'  # Đặt đường dẫn tới ảnh đầu vào
    ...
    if len(corners) == 4:
        for idx, (corner_x, corner_y) in enumerate(corners):
            print(f"Corner {idx + 1}: ({corner_x}, {corner_y})")
        label_answer = create_label_answer()
        # Cắt và làm thẳng ảnh
        cropped_img, new_corner_coord = crop_img(frame, corners)
        warped_img = warp_perspective(cropped_img, new_corner_coord)
        binary_image = histogram_equalization_and_binarization("warped_final.jpg")
        dilated_image = morphological_transform(binary_image)
        all_keypoints = detect_white_point(dilated_image)
        grouped_keypoints = group_points_by_y(all_keypoints, 30)
        section_key_point = remove_4_corners(grouped_keypoints)
        sheet_process_result = process_bubble_sheet(warped_img, section_key_point)
        save_results(sheet_process_result, 'output')
        student_id, exam_code, final_answer = extract_information(line_model)
        score = calculate_score(final_answer, label_answer)
        ...
        print("Không tìm thấy đủ 4 góc của phiếu chấm thi.")
